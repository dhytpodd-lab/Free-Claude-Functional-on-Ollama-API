...
For Ollama Cloud / direct `ollama.com` API access, set:
...
`OLLAMA_BASE_URL` is the Ollama server root; do not append `/api` or `/v1`. The provider calls `/api/tags` for model discovery and `/v1/messages` for Anthropic-compatible Claude Code traffic. Use the exact model tag shown by the Ollama provider card, prefixed with `ollama/` (for example `ollama/deepseek-v3`).

For local Ollama instead, run Ollama and pull a model:
