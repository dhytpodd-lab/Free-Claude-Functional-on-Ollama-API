...
async def local_provider_status(request: Request):
    ...
    for provider_id, path in LOCAL_PROVIDER_PATHS.items():
        ...
        headers = _local_provider_headers(provider_id, values)
        checks.append(
            await _check_local_provider(provider_id, base_url, path, headers=headers)
        )
...
def _local_provider_headers(provider_id: str, values: dict[str, str]) -> dict[str, str]:
    if provider_id != "ollama":
        return {}
...
async def _check_local_provider(
    provider_id: str, base_url: str, path: str, *, headers: dict[str, str] | None = None
) -> dict[str, Any]:
